...
@auth
    ...
    <div x-data="{ 
        ...
        init() {
            // Listen for Livewire event to update counter
            window.Livewire.on('notification-read', (data) => {
                this.unreadCount = Math.max(0, this.unreadCount - 1);
            });
            
            window.Livewire.on('notifications-marked-as-read', (data) => {
                this.unreadCount = 0;
            });
        }
    }" class="relative">
        <button 
            @click="show = !show; $wire.loadNotifications()" 
                            ...
                            @foreach(auth()->user()->notifications()->latest()->limit(10)->get() as $notification)
                                <li class="border-b border-zinc-200 dark:border-zinc-700 last:border-0">
                                    <a 
                                        ...
                                        class="block p-3 hover:bg-zinc-50 dark:hover:bg-zinc-700 {{ $notification->read_at ? '' : 'bg-blue-50 dark:bg-zinc-700' }}"
                                    >
