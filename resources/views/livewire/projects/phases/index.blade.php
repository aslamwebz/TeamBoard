<div class="max-w-6xl mx-auto p-6">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-3xl font-bold text-foreground">{{ $project->name }} Phases</h1>
            <p class="text-muted-foreground">Manage phases for this project</p>
        </div>
        <div class="flex gap-3">
            <flux:button :href="route('projects.show', $project)" variant="outline">
                Back to Project
            </flux:button>
            <flux:button :href="route('phases.create', $project)">
                + Add Phase
            </flux:button>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg border border-zinc-200 dark:border-zinc-700 overflow-hidden">
                ...
                <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                    <thead class="bg-zinc-50 dark:bg-zinc-700">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-300 uppercase tracking-wider">Start Date</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-300 uppercase tracking-wider">End Date</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-zinc-500 dark:text-zinc-300 uppercase tracking-wider">Progress</th>
                        ...
                        @foreach($phases as $phase)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-700/50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-foreground">{{ $phase->name }}</div>
                                    @if($phase->description)
                                        <div class="text-sm text-muted-foreground">{{ Str::limit($phase->description, 50) }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @php
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-foreground">
                                    {{ $phase->start_date ? $phase->start_date->format('M d, Y') : 'N/A' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-foreground">
                                    {{ $phase->end_date ? $phase->end_date->format('M d, Y') : 'N/A' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
        ...
        @else
            <div class="p-12 text-center">
                ...
                <h3 class="mt-2 text-sm font-medium text-foreground">No phases</h3>
                <p class="mt-1 text-sm text-muted-foreground">Get started by creating a new phase.</p>
                <div class="mt-6">
                    <flux:button :href="route('phases.create', $project)">
